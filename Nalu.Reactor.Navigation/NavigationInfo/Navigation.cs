...
public abstract class Navigation : BindableObject, IList<INavigationSegment>, INavigationInfo
{
    ...
    {
        if (value?.IsSubclassOf(typeof(MauiReactor.Component)) != true)
        {
            throw new InvalidOperationException("PageType must be a type that inherits from MauiReactor.Component.");
        }
    ...
    /// </summary>
    public static readonly BindableProperty PropsDelegateProperty = BindableProperty.Create(
        nameof(PropsDelegate),
    ...
    /// <inheritdoc />
    public Action<object>? PropsDelegate
    {
        get => (Action<object>?)GetValue(PropsDelegateProperty);
        set => SetValue(PropsDelegateProperty, value);
    }
    ...
    public override string ToString()
    {
        ...
        return $"{Path}?({PropsDelegate.GetType().Name})";
    }
    ...
    private static void PageTypePropertyChanged(BindableObject bindable, object oldvalue, object newvalue)
    {
        ...
        {
            throw new InvalidOperationException("ShellPage must be a type");
        }
