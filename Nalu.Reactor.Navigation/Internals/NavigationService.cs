...
internal partial class NavigationService : INavigationService, IDisposable
{
    ...
    public async Task InitializeAsync(IShellProxy shellProxy, string contentSegmentName)
    {
        ...
        var (component, page) = content.GetOrCreateContent();

        var enteringTask = NavigationHelper.SendEnteringAsync(ShellProxy, component, Configuration).AsTask();
        if (!enteringTask.IsCompleted)
        {
            throw new NotSupportedException($"OnEnteringAsync() must not be async for the initial page {page!.GetType().FullName}.");
        }
...
#pragma warning restore VSTHRD002
        ...
        await NavigationHelper.SendAppearingAsync(ShellProxy, component, Configuration).ConfigureAwait(true);
    }
    ...
    public async Task<bool> GoToAsync(INavigationInfo navigation)
    {
        ...
        return await ExecuteNavigationAsync(
                ...
                async initialState =>
                {
                    ...
                    var (requestedNavigation, targetState) = ComputeNavigationState(navigation, initialState);
    ...
    private void ConfigureBackButtonBehavior(MauiReactor.Component page, bool isRoot)
    {
        ...
        page.SetProperty(Shell.BackButtonBehaviorProperty, backButtonBehavior);
    }
    ...
    )
    {
        ...
        for (var i = 0; i < navigationCount; i++)
        {
            ...
            if (isPop)
            {
                var isGuarded = !ignoreGuards && stackPage.PageComponent is ILeavingGuard;
                ...
                if (isGuarded)
                {
                    ...
                    await NavigationHelper.SendAppearingAsync(ShellProxy, stackPage.PageComponent, Configuration).ConfigureAwait(true);
                    var canLeave = await NavigationHelper.CanLeaveAsync(ShellProxy, stackPage.PageComponent).ConfigureAwait(true);
                ...
                await NavigationHelper.SendDisappearingAsync(ShellProxy, stackPage.PageComponent).ConfigureAwait(true);
                await NavigationHelper.SendLeavingAsync(ShellProxy, stackPage.PageComponent).ConfigureAwait(true);
            ...
            {
                await NavigationHelper.SendDisappearingAsync(ShellProxy, stackPage.PageComponent).ConfigureAwait(true);
                var pageType = NavigationHelper.GetPageType(segment.Type);
                ...
                extern static ref Dictionary<BindableProperty, object?> GetAttachedPropertiesField(MauiReactor.Component @this);

                var presentationMode = GetAttachedPropertiesField(component).GetValueOrDefault(Shell.PresentationModeProperty);
                var isModal = presentationMode is not default(object) && ((PresentationMode)presentationMode).HasFlag(PresentationMode.Modal);
                await NavigationHelper.SendEnteringAsync(ShellProxy, component, Configuration).ConfigureAwait(true);
                await shellProxy.PushAsync(segmentName, component, navigation.PropsDelegate).ConfigureAwait(true);
                stack.Add(new NavigationStackPage($"{stackPage.Route}/{segmentName}", segmentName, component, isModal));
            }
        }
        ...
        if (sendAppearingToTarget)
        {
            ...
            await NavigationHelper.SendAppearingAsync(ShellProxy, page.PageComponent, Configuration, navigation.PropsDelegate).ConfigureAwait(true);
        }
    ...
    private async Task<bool> ExecuteAbsoluteNavigationAsync(INavigationInfo navigation, HashSet<object> disposeBag, bool ignoreGuards)
    {
        ...
        var rootSegmentName = NavigationHelper.GetSegmentName(rootSegment);
        ...
        var navigationStack = currentSection.GetNavigationStack(ServiceProvider).ToList();
    ...
    )
    {
        ...
        foreach (var sectionContentsToLeave in groupedContentsToLeave)
        {
            foreach (var contentToLeave in sectionContentsToLeave)
            {
                ...
                {
                    var navigationStackItem = navigationStack[^1];

                    await EnsureContentIsSelectedAsync().ConfigureAwait(true);
                    await NavigationHelper.SendDisappearingAsync(ShellProxy, navigationStackItem.PageComponent).ConfigureAwait(true);
                ...
                if (!ignoreGuards && component is ILeavingGuard)
                {
                    ...
                    if (!await NavigationHelper.CanLeaveAsync(ShellProxy, component).ConfigureAwait(true))
                    {
                        return false;
                    }
                }

                await NavigationHelper.SendDisappearingAsync(ShellProxy, component).ConfigureAwait(true);
                await NavigationHelper.SendLeavingAsync(ShellProxy, component).ConfigureAwait(true);
        ...
        var targetSection = targetContent.Parent;
        var targetStack = targetSection.GetNavigationStack(ServiceProvider, targetContent).ToList();
        ...
        await shellProxy.CommitNavigationAsync().ConfigureAwait(true);
        await NavigationHelper.SendAppearingAsync(ShellProxy, targetComponent, Configuration).ConfigureAwait(true);

        return true;
    }

    private static RelativeNavigation ToRelativeNavigation(INavigationInfo navigation, IReadOnlyList<NavigationStackPage> navigationStackPages)
    {
        var matchingSegmentsCount = navigation
            .Skip(1)
            .Select(NavigationHelper.GetSegmentName)
        ...
        if (navigation.PropsDelegate is { } intent && relativeNavigation.Count > 0)
        {
            relativeNavigation.WithPropsDelegate(intent);
        }
    ...
    private static (string RequestedNavigation, string TargetState) ComputeNavigationState(INavigationInfo navigation, string initialState)
    {
        ...
        if (navigation.PropsDelegate is { } intent)
        {
